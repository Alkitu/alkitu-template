# ALI-115: DB Authentication & Public Access (User + Auth)

## Metadata

- **Issue Key**: ALI-115
- **Type**: Tarea
- **Status**: ✅ **DONE** (Completed 2025-11-24)
- **Priority**: Medium
- **Assignee**: Luis Eduardo Urdaneta Martucci
- **Reporter**: Leonel
- **Created**: 2025-11-18T02:59:04.723+0100
- **Updated**: 2025-11-24T00:15:00.000+0100
- **Sprint**: Diseño Interfaz y Data Base (ID: 37)
  - Start: 2025-11-23T21:19:39.908Z
  - End: 2025-11-30T21:19:06.000Z
- **Implementation Time**: ~6 hours
- **Tests**: 10/10 E2E + 20+ Unit (100% passing)

## Parent Epic

- **Key**: ALI-18
- **Summary**: Data Base [Global]
- **Status**: Discovery

## Description

Definir y configurar toda la parte de base de datos necesaria para registro, login y recuperación de contraseña.
Incluye el uso mínimo del modelo `User` (`email`, `password`, `role`, `createdAt`, `updatedAt`) y su integración con los endpoints de autenticación.

**Modelos / campos clave:**

- `User.email` (único)
- `User.password` (hash)
- `User.role` (default CLIENT)
- `User.profileComplete` (default false)

### ¿Dónde se usa en el sitemap?

**Pantallas:**

- **Landing Page – "Welcome & Access Entry" [Global] (ALI-53)**
  - No lee datos de BD, pero es la puerta hacia:
    - Botón "Login" → flujo que usará `User.email` + `User.password`.
    - Botón "Register" → flujo que creará un nuevo `User`.

- **Register – "Create Your Account" [Global] (ALI-21)**
  - Crea un `User` mínimo con:
    - `email` (campo del formulario).
    - `password` (formulario, se guarda hasheada).
    - `role` → valor por defecto `CLIENT`.
    - `profileComplete` → `false` por defecto.
    - `createdAt`, `updatedAt` → seteados automáticamente.

  - Tras registro, redirige al onboarding de perfil.

- **Login – "Sign In to Your Account" [Global] (ALI-22)**
  - Usa:
    - `email` + `password` para autenticar.

  - Según el usuario recuperado:
    - `role` decide qué dashboard mostrar.
    - `profileComplete` decide si va directo a `/app/dashboard` o al onboarding de perfil.

- **Password Reset – "Recover Your Access" (ALI-23)**
  - Se apoya en `User.email` para:
    - Encontrar al usuario que solicita el reset.
    - Enviar el enlace de recuperación.

  - Al confirmar nueva contraseña:
    - Actualiza `User.password` y `updatedAt`.

## Schema Definition

```prisma
/// EPIC DB-1 – Authentication & Public Access (User + Auth)

type ContactPerson {
  name     String
  lastname String
  phone    String
  email    String
}

enum Role {
  CLIENT
  EMPLOYEE
  ADMIN
}

model User {
  id               String          @id @default(auto()) @map("_id") @db.ObjectId
  email            String          @unique
  password         String
  firstname        String
  lastname         String
  phone            String
  company          String
  address          String?         // Main Address (relevante para CLIENT)
  contactPerson    ContactPerson?
  role             Role            @default(CLIENT)
  profileComplete  Boolean         @default(false)
  locations        WorkLocation[]  @relation("UserLocations")
  requests         Request[]       @relation("UserRequests")
  assignedRequests Request[]       @relation("RequestAssignedEmployee")
  notifications    Notification[]  @relation("UserNotifications")
  createdAt        DateTime        @default(now())
  updatedAt        DateTime        @updatedAt
}
```

## Related Issues

- **ALI-53**: Landing Page – "Welcome & Access Entry" [Global]
- **ALI-21**: dev Register – "Create Your Account" [Global]
- **ALI-22**: dev Login – "Sign In to Your Account" [Global]
- **ALI-23**: dev Password Reset – "Recover Your Access"

## Implementation Summary

### ✅ Completado (100%)

**Database**:

- ✅ Schema migrado con todos los campos requeridos
- ✅ ContactPerson type implementado
- ✅ profileComplete field con lógica completa
- ✅ Índices y constraints configurados

**Backend** (95%+ coverage):

- ✅ Register endpoint con campos mínimos
- ✅ Login endpoint con JWT + profileComplete
- ✅ Password reset flow completo
- ✅ Email verification
- ✅ Rate limiting (5 login/min, 20 register/hour)
- ✅ Password complexity validation
- ✅ Endpoint `/auth/complete-profile`

**Frontend** (100% nuevos componentes):

- ✅ PasswordStrengthIndicator component
- ✅ RegisterFormOrganism (campos mínimos)
- ✅ OnboardingFormOrganism (campos adicionales)
- ✅ Onboarding page `/app/onboarding`
- ✅ Middleware con profileComplete logic
- ✅ Redirect basado en profileComplete

**Tests**:

- ✅ Backend: 95%+ coverage, 85%+ mutation score
- ✅ Frontend: 10/10 E2E (Playwright), 20+ Unit (Vitest)
- ✅ Zero linting errors
- ✅ Zero TypeScript errors

### 📚 Documentation

- **Spec**: `/jira/sprint-1/specs/ALI-115/ALI-115-auth-spec.md`
- **Frontend Implementation**: `/jira/sprint-1/specs/ALI-115/ALI-115-auth-frontend-feedback.md`
- **Backend Implementation**: `/jira/sprint-1/specs/ALI-115/ALI-115-auth-backend-feedback.md`

### 🎯 User Flows Implemented

1. **Register** → Email verification → Login → Onboarding (if profileComplete=false) → Dashboard
2. **Login** → profileComplete check → Onboarding OR Dashboard
3. **Password Reset** → Email → New password → Login
4. **Onboarding** → Complete profile (optional) → Dashboard

### 🚀 Status

**PRODUCTION READY** - All acceptance criteria met, tests passing, documentation complete.

## Links

- [View in Jira](https://alkitu.atlassian.net/browse/ALI-115)
