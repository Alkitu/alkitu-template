...
import { Channel, ChannelMember, ChannelType, ChannelRole } from '@prisma/client';
...
export class ChannelRepository {
  ...
  async create(data: CreateChannelDto & { workspaceId?: string }, ownerId: string): Promise<Channel> {
  ...
  async addMember(channelId: string, userId: string, role: ChannelRole = ChannelRole.MEMBER): Promise<ChannelMember> {
  ...
  async findOrCreateConversation(creatorId: string, targetUserIds: string[]): Promise<Channel> {
    ...
    const existing = candidateChannels.find(c => {
      ...
      return memberIds.length === participantIds.length && 
