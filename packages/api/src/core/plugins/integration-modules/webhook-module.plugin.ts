...
export class WebhookModulePlugin implements IModulePlugin {
  ...
  async create(config: ModuleFlag): Promise<any> {
    try {
      ...
      const webhookModule = {
        ...
        registerWebhook: (webhook: WebhookConfig) =>
        ...
        validateWebhook: (webhook: WebhookConfig) =>
  ...
  private async getWebhooks(userId?: string): Promise<RegisteredWebhook[]> {
    await Promise.resolve(); // ESLint requires await in async methods
    this.logger.debug(
    ...
    const mockWebhooks = [
      ...
      },
      {
        ...
        active: true,
        secret: 'wh_secret_abcdef1234567890',
  ...
  private validateWebhook(webhook: WebhookConfig): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
  ...
  private async retryFailedWebhooks(): Promise<{
