...
export class WebSocketController {
  ...
  async getWebSocketStatus(): Promise<{
    ...
    const userSocketsMap = this.notificationGateway.getUserSockets();
  ...
  async getActiveConnections(): Promise<{
    ...
    for (const [userId, socketIds] of userSocketsMap.entries()) {
      ...
      const firstSocket = server.sockets.sockets.get(socketIdsArray[0]);
  ...
  async broadcastMessage(): Promise<{
    message: string;
