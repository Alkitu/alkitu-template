...
export interface IAuthenticationService extends IBaseService {
   ...
   */
  authenticate(credentials: LoginCredentials): Promise<ServiceResult<AuthenticationResult>>;
   ...
   */
  refreshToken(refreshToken: string): Promise<ServiceResult<{ accessToken: string; expiresIn: number }>>;
   ...
   */
  terminateSession(userId: string, sessionId: string): Promise<ServiceResult<void>>;
   ...
   */
  getAuthenticationAttempts(userId: string, limit?: number): Promise<ServiceResult<AuthenticationAttempt[]>>;
}