...
export class MarketingEmailChannel implements IEmailChannel {
  ...
  async send(data: MarketingEmailData): Promise<EmailResult> {
    try {
      this.logger.log(`Sending marketing email to ${data.userEmail} for campaign: ${data.campaignName}`);
   ...
   */
  private enhanceMarketingData(data: MarketingEmailData): MarketingEmailData & { trackingUrl?: string } {
