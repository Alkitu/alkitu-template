...
import {
  ...
  IMetrics,
  IEventBus,
} from '../../common/interfaces/infrastructure.interface';
  ...
  implements IUserManagementService
{
  ...
  async cleanup(): Promise<ServiceResult<void>> {
    try {
      ...
      this.logger.error(
        'User Management Service cleanup failed',
        error as Error,
        {
          operation: 'cleanup',
          metadata: {
  ...
  async createUser(userData: UserData): Promise<ServiceResult<User>> {
    ...
    try {
      ...
      const emailAvailable = await this.userRepository.findByEmail(
        userData.email,
      );
      if (emailAvailable.success && emailAvailable.data) {
        ...
        return {
          success: false,
          error: new ValidationError('Email address is already in use', {
            email: userData.email,
          }),
        };
      }
  ...
  async updateUser(
    ...
    try {
      ...
      const updateResult = await this.userRepository.update(userId, {
        ...
        ...(updates.metadata && {
          metadata: { ...existingUser.metadata, ...updates.metadata },
        }),
      });
  ...
  async getUserById(userId: string): Promise<ServiceResult<User>> {
    try {
      ...
      return {
        success: false,
        error: new ServiceUnavailableError('Failed to retrieve user', {
          originalError:
            error instanceof Error ? error.message : 'Unknown error',
          userId,
        }),
  ...
  async listUsers(
    ...
    try {
      ...
      return {
        success: false,
        error: new ServiceUnavailableError('Failed to list users', {
          originalError:
            error instanceof Error ? error.message : 'Unknown error',
        }),
      };
  ...
  async userExists(userId: string): Promise<ServiceResult<boolean>> {
    try {
      ...
      return {
        success: false,
        error: new ServiceUnavailableError('Failed to check user existence', {
          originalError:
            error instanceof Error ? error.message : 'Unknown error',
          userId,
        }),
  ...
  private validateUserData(userData: UserData): {
    ...
    if (!userData.email || typeof userData.email !== 'string') {
      errors.push(
        new ValidationError('Email is required and must be a string'),
      );
    ...
    if (!userData.firstName || typeof userData.firstName !== 'string') {
      errors.push(
        new ValidationError('First name is required and must be a string'),
      );
    }

    if (!userData.lastName || typeof userData.lastName !== 'string') {
      errors.push(
        new ValidationError('Last name is required and must be a string'),
      );
    }

    if (!userData.password || typeof userData.password !== 'string') {
      errors.push(
        new ValidationError('Password is required and must be a string'),
      );
    ...
    if (
      userData.role &&
      !['user', 'admin', 'moderator'].includes(userData.role)
    ) {
      errors.push(
        new ValidationError('Role must be one of: user, admin, moderator'),
      );
    }
