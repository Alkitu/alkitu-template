...
export interface ServiceRegistration {
  ...
  registerFactory<T>(token: string | symbol | Function, factory: (container: DIContainer) => T, lifetime?: ServiceLifetime): void;
}
...
export interface ServiceLifetimeBuilder<T> {
  ...
  asTransient(): void;
  asScoped(): void;
  withDependencies(...dependencies: (string | symbol | Function)[]): ServiceLifetimeBuilder<T>;
}
...
export class DIContainer implements ServiceRegistration, ServiceResolution, ContainerManagement {
  ...
  isRegistered(token: string | symbol | Function): boolean {
    return this.services.has(token) || (this.parent ? this.parent.isRegistered(token) : false);
  }
  ...
  private getServiceDescriptor(token: string | symbol | Function): ServiceDescriptor | null {
...
class ServiceRegistrationBuilderImpl<T> implements ServiceRegistrationBuilder<T> {
  constructor(
  ...
  to(implementation: new (...args: any[]) => T): ServiceLifetimeBuilder<T> {
    return new ServiceLifetimeBuilderImpl<T>(this.container, this.token, { implementation });
  }

  toFactory(factory: (container: DIContainer) => T): ServiceLifetimeBuilder<T> {
    return new ServiceLifetimeBuilderImpl<T>(this.container, this.token, { factory });
  }
...
class ServiceLifetimeBuilderImpl<T> implements ServiceLifetimeBuilder<T> {
  ...
  constructor(
    private container: DIContainer,
    private token: string | symbol | Function,
    options: { implementation?: new (...args: any[]) => T; factory?: (container: DIContainer) => T }
  ) {
    this.descriptor = {
      token: this.token,
      ...options,
  ...
  private finalize(): void {
    (this.container as any).internal_setServiceDescriptor(this.descriptor as ServiceDescriptor<T>);
  }
}
...
export class ContainerError extends Error {
  constructor(message: string, public readonly token?: string | symbol | Function) {
    super(message);
...
export function isDisposable(obj: any): obj is { dispose(): Promise<void> | void } {
