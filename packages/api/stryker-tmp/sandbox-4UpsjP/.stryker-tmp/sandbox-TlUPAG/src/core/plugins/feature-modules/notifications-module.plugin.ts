...
export class NotificationsModulePlugin implements IModulePlugin {
  ...
  async create(config: ModuleFlag): Promise<any> {
    try {
      this.logger.log('Creating Notifications module with config:', config.config);
  ...
  validateConfig(config: ModuleFlag): { isValid: boolean; errors: string[] } {
    ...
    if (!config.config) {
      ...
      if (config.config.enableEmail && typeof config.config.enableEmail !== 'boolean') {
        errors.push('enableEmail must be a boolean');
      }

      if (config.config.enablePush && typeof config.config.enablePush !== 'boolean') {
        errors.push('enablePush must be a boolean');
      }

      if (config.config.enableInApp && typeof config.config.enableInApp !== 'boolean') {
        errors.push('enableInApp must be a boolean');
      }

      if (config.config.retentionDays && (!Number.isInteger(config.config.retentionDays) || config.config.retentionDays < 1)) {
  ...
  getMetadata(): ModuleMetadata {
    return {
      ...
      description: 'Multi-channel notification system supporting email, push, and in-app notifications',
      configSchema: {
        enableEmail: { type: 'boolean', required: false, description: 'Enable email notifications' },
        enablePush: { type: 'boolean', required: false, description: 'Enable push notifications' },
        enableInApp: { type: 'boolean', required: false, description: 'Enable in-app notifications' },
        retentionDays: { type: 'number', required: false, description: 'Notification retention period in days' },
        channels: { type: 'array', required: false, description: 'Available notification channels' },
        defaultChannel: { type: 'string', required: false, description: 'Default notification channel' },
      },
