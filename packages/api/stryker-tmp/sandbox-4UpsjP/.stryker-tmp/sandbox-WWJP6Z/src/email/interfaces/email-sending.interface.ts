...
export interface IEmailSendingService extends IBaseService {
   ...
   */
  sendEmail(emailMessage: EmailMessage): Promise<ServiceResult<EmailSendResult>>;
   ...
   */
  sendBulkEmails(emailMessages: EmailMessage[]): Promise<ServiceResult<BulkEmailJob>>;
   ...
   */
  sendBroadcastEmail(recipients: string[], emailMessage: Omit<EmailMessage, 'to'>): Promise<ServiceResult<EmailSendResult>>;
   ...
   */
  validateEmailMessage(emailMessage: EmailMessage): Promise<ServiceResult<EmailValidation>>;
   ...
   */
  getDeliveryStatus(messageId: string): Promise<ServiceResult<{ status: 'sent' | 'delivered' | 'bounced' | 'failed'; timestamp: Date; details?: string }>>;
}