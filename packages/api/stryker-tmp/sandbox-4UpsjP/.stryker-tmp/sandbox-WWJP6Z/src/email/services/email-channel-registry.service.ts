...
import { IEmailChannel, IEmailChannelRegistry } from '../channels/email-channel.interface';
 ...
 */
@Injectable()
export class EmailChannelRegistryService implements IEmailChannelRegistry, OnModuleInit {
  ...
  async sendEmail(type: string, data: any): Promise<EmailResult> {
    ...
    try {
      this.logger.log(`Sending '${type}' email using ${channel.constructor.name}`);
      const result = await channel.send(data);
      
      if (result.success) {
        this.logger.log(`Email '${type}' sent successfully with messageId: ${result.messageId}`);
      } else {
  ...
  unregisterChannel(type: string): boolean {
    ...
    if (existed) {
      ...
      this.logger.warn(`Attempted to unregister non-existent email channel: ${type}`);
    }
