...
describe('TokenService', () => {
  ...
  beforeEach(async () => {
    ...
    (prismaService.passwordResetToken.findFirst as any).mockResolvedValue({
      id: 'token-id',
      token: 'reset-token-123',
      userId: 'user-id',
      expiresAt: new Date(Date.now() + 3600000),
    ...
    (prismaService.verificationToken.findFirst as any).mockResolvedValue({
      id: 'token-id',
      token: 'verify-token-123',
      userId: 'user-id',
      expiresAt: new Date(Date.now() + 3600000),
    ...
    (prismaService.refreshToken.findFirst as any).mockResolvedValue({
      id: 'token-id',
      token: 'refresh-token-123',
      userId: 'user-id',
      expiresAt: new Date(Date.now() + 86400000),
