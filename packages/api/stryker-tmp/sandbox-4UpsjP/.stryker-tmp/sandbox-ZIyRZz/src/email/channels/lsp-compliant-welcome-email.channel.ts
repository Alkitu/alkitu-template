...
export class LSPCompliantWelcomeEmailChannel implements IEmailChannel<WelcomeEmailData> {
  ...
  readonly supportedDataTypes = ['welcome'] as const;
  ...
  async send(data: WelcomeEmailData): Promise<ServiceResult<EmailDeliveryResult>> {
    ...
    try {
      ...
      if (emailResult.success) {
        ...
        this.logger.log(`Welcome email sent successfully to ${data.recipientEmail}, messageId: ${deliveryResult.messageId}`);
  ...
  validateData(data: WelcomeEmailData): ValidationResult {
    ...
    try {
      ...
      if (!data.recipientEmail || typeof data.recipientEmail !== 'string') {
        errors.push(new ValidationError('recipientEmail is required and must be a string'));
      } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.recipientEmail)) {
        errors.push(new ValidationError('recipientEmail must be a valid email address'));
      }

      if (!data.recipientName || typeof data.recipientName !== 'string') {
        errors.push(new ValidationError('recipientName is required and must be a string'));
      }
      ...
      if (!data.registrationDate || typeof data.registrationDate !== 'string') {
        errors.push(new ValidationError('registrationDate is required and must be a string'));
      }

      if (!data.loginUrl || typeof data.loginUrl !== 'string') {
        errors.push(new ValidationError('loginUrl is required and must be a string'));
      ...
      if (!data.unsubscribeUrl || typeof data.unsubscribeUrl !== 'string') {
        errors.push(new ValidationError('unsubscribeUrl is required and must be a string'));
      ...
      if (!data.supportUrl || typeof data.supportUrl !== 'string') {
        errors.push(new ValidationError('supportUrl is required and must be a string'));
      ...
      if (data.senderName && typeof data.senderName !== 'string') {
        errors.push(new ValidationError('senderName must be a string if provided'));
      }

      if (data.replyTo && (typeof data.replyTo !== 'string' || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.replyTo))) {
        errors.push(new ValidationError('replyTo must be a valid email address if provided'));
      }
    ...
    return {
      ...
      errors,
      warnings: errors.length > 0 ? [`Found ${errors.length} validation errors`] : undefined,
    };
  }
