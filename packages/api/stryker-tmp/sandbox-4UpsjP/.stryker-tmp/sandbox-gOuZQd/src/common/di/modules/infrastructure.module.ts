...
class InMemoryFileStorage implements IFileStorage {
  ...
  async upload(key: string, data: Buffer, options?: UploadOptions): Promise<ServiceResult<FileMetadata>> {
  ...
  async download(key: string): Promise<ServiceResult<{ data: Buffer; metadata: FileMetadata }>> {
  ...
  async getSignedUrl(key: string, expiresIn?: number): Promise<ServiceResult<string>> {
  ...
  async listFiles(prefix?: string, limit?: number): Promise<ServiceResult<FileMetadata[]>> {
...
class CloudFlareR2Storage implements IFileStorage {
  constructor(private config: IConfigurationProvider, private logger: ILogger) {}
...
class AxiosHttpClient implements IHttpClient {
  private requestInterceptors: ((config: HttpRequestOptions) => HttpRequestOptions)[] = [];
  private responseInterceptors: ((response: HttpResponse<any>) => HttpResponse<any>)[] = [];
  ...
  async get<T>(url: string, options?: HttpRequestOptions): Promise<ServiceResult<HttpResponse<T>>> {
    return this.makeRequest<T>('GET', url, undefined, options);
  }

  async post<T, D = any>(url: string, data?: D, options?: HttpRequestOptions): Promise<ServiceResult<HttpResponse<T>>> {
    return this.makeRequest<T>('POST', url, data, options);
  }

  async put<T, D = any>(url: string, data?: D, options?: HttpRequestOptions): Promise<ServiceResult<HttpResponse<T>>> {
    return this.makeRequest<T>('PUT', url, data, options);
  }

  async patch<T, D = any>(url: string, data?: D, options?: HttpRequestOptions): Promise<ServiceResult<HttpResponse<T>>> {
    return this.makeRequest<T>('PATCH', url, data, options);
  }

  async delete<T>(url: string, options?: HttpRequestOptions): Promise<ServiceResult<HttpResponse<T>>> {
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      this.logger.error(`[HTTP Client] Request failed: ${method} ${url}`, error as Error);
...
class InMemoryQueue implements IQueue {
  ...
  async add<TData>(jobType: string, data: TData, options?: QueueOptions): Promise<ServiceResult<QueueJob<TData>>> {
  ...
  async process<TData>(jobType: string, processor: JobProcessor<TData>): Promise<ServiceResult<void>> {
  ...
  async getJobCounts(): Promise<ServiceResult<{ waiting: number; active: number; completed: number; failed: number }>> {
    return {
      success: true,
      data: {
...
class SimpleQueue implements IQueue {
  constructor(private logger: ILogger) {}
