...
import { IConversationRepository, IMessageRepository, IContactInfoRepository, ConversationResult, MessageResult } from './interfaces/chat.interface';
...
export class ChatService {
  ...
  private sanitizeContactInfo(contactInfo: any): any {
  ...
  async startConversation(data: StartConversationDto): Promise<ConversationResult> {
    let contactInfo = await this.contactInfoRepository.findByEmail(data.email || '');
    if (!contactInfo) {
  ...
  async getConversations(filter: GetConversationsDto): Promise<any[]> {
    const where: any = {};
    if (filter.status) where.status = filter.status;
    if (filter.priority) where.priority = filter.priority;
    if (filter.search) {
      where.OR = [
        { 'contactInfo.email': { contains: filter.search, mode: 'insensitive' } },
        { 'contactInfo.name': { contains: filter.search, mode: 'insensitive' } },
        { 'messages.content': { contains: filter.search, mode: 'insensitive' } },
      ];
    }
    ...
    return conversations.map(this.sanitizeConversation);
  }

  async assignConversation(data: AssignConversationDto): Promise<any> {
    const conversation = await this.conversationRepository.findById(data.conversationId);
    if (!conversation) throw new NotFoundException('Conversation not found');
    return this.conversationRepository.update(data.conversationId, { assignedToId: data.assignedToId });
  }

  async updateStatus(data: UpdateStatusDto): Promise<any> {
    const conversation = await this.conversationRepository.findById(data.conversationId);
    if (!conversation) throw new NotFoundException('Conversation not found');
    return this.conversationRepository.update(data.conversationId, { status: data.status });
  }
  ...
  async addInternalNote(data: AddInternalNoteDto): Promise<any> {
    const conversation = await this.conversationRepository.findById(data.conversationId);
    if (!conversation) throw new NotFoundException('Conversation not found');
    const newNotes = conversation.internalNotes ? `${conversation.internalNotes}\n${data.note}` : data.note;
    return this.conversationRepository.update(data.conversationId, { internalNotes: newNotes });
  }
  ...
  async markAsRead(data: MarkAsReadDto): Promise<void> {
    ...
    await this.prisma.chatMessage.updateMany({
      where: { conversationId: data.conversationId, isFromVisitor: true, isRead: false },
      data: { isRead: true },
    });
  }
  ...
  async getChatAnalytics(days: number = 30) {
    ...
    const [totalConversations, openConversations, resolvedConversations, leadsCaptured] = await Promise.all([
      this.conversationRepository.findAll({ where: { createdAt: { gte: startDate } } }),
      this.conversationRepository.findAll({ where: { status: ConversationStatus.OPEN, createdAt: { gte: startDate } } }),
      this.conversationRepository.findAll({ where: { status: ConversationStatus.RESOLVED, createdAt: { gte: startDate } } }),
      this.contactInfoRepository.findAll({ where: { createdAt: { gte: startDate } } }),
    ]);
