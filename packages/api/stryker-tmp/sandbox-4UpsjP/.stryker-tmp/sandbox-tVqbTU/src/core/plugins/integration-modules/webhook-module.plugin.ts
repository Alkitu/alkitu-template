...
import { IModulePlugin, ModuleCategory, ModuleMetadata } from '../module-plugin.interface';
...
export class WebhookModulePlugin implements IModulePlugin {
  ...
  async create(config: ModuleFlag): Promise<any> {
    try {
      ...
      const webhookModule = {
        ...
        triggerWebhook: (event: string, data: any) => this.triggerWebhook(event, data),
  ...
  validateConfig(config: ModuleFlag): { isValid: boolean; errors: string[] } {
    ...
    if (!config.config) {
      ...
      if (config.config.maxRetries && (!Number.isInteger(config.config.maxRetries) || config.config.maxRetries < 0)) {
        errors.push('maxRetries must be a non-negative integer');
      }

      if (config.config.timeoutMs && (!Number.isInteger(config.config.timeoutMs) || config.config.timeoutMs < 1000)) {
        errors.push('timeoutMs must be an integer >= 1000');
      }

      if (config.config.enableSigning && typeof config.config.enableSigning !== 'boolean') {
        errors.push('enableSigning must be a boolean');
      }

      if (config.config.signingSecret && typeof config.config.signingSecret !== 'string') {
        errors.push('signingSecret must be a string');
      }

      if (config.config.allowedEvents && !Array.isArray(config.config.allowedEvents)) {
        errors.push('allowedEvents must be an array');
      }

      if (config.config.rateLimit && typeof config.config.rateLimit !== 'object') {
  ...
  getMetadata(): ModuleMetadata {
    return {
      ...
      description: 'Webhook integration module providing event-driven HTTP callbacks to external services',
      configSchema: {
        maxRetries: { type: 'number', required: false, description: 'Maximum retry attempts for failed webhooks' },
        timeoutMs: { type: 'number', required: false, description: 'Webhook request timeout in milliseconds' },
        enableSigning: { type: 'boolean', required: false, description: 'Enable webhook payload signing' },
        signingSecret: { type: 'string', required: false, description: 'Secret key for webhook payload signing' },
        allowedEvents: { type: 'array', required: false, description: 'List of events that can trigger webhooks' },
        rateLimit: { type: 'object', required: false, description: 'Rate limiting configuration for webhooks' },
        enableLogging: { type: 'boolean', required: false, description: 'Enable detailed webhook logging' },
      },
  ...
  private async registerWebhook(webhook: any): Promise<any> {
    ...
    this.logger.log(`Webhook registered: ${registeredWebhook.id} -> ${registeredWebhook.url}`);
  ...
  private async triggerWebhook(event: string, data: any): Promise<any> {
    ...
    const webhookResult = {
      ...
      results: [
        ...
        { webhookId: 'webhook_3', status: 'failed', error: 'Connection timeout' },
      ],
    };

    this.logger.log(`Webhook event '${event}' processed: ${webhookResult.successful}/${webhookResult.webhooksTriggered} successful`);
    return webhookResult;
  }

  private async getWebhooks(userId?: string): Promise<any[]> {
    this.logger.debug(`Getting webhooks${userId ? ` for user: ${userId}` : ''}`);
    ...
    return userId ? mockWebhooks.filter(w => w.userId === userId) : mockWebhooks;
  }

  private validateWebhook(webhook: any): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  private async retryFailedWebhooks(): Promise<any> {
    ...
    this.logger.log(`Webhook retry completed: ${retryResult.successful}/${retryResult.retriedWebhooks} now successful`);
    return retryResult;
  }
