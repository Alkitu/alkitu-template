...
export class ModuleManagerService implements OnModuleInit {
  ...
  private async createModuleUsingRegistry(name: string, config: ModuleFlag): Promise<any> {
    try {
      ...
      this.logger.log(`Module '${name}' created successfully via plugin registry`);
      return moduleInstance;
    } catch (error) {
      this.logger.error(`Failed to create module '${name}' via plugin registry:`, error);
      
      // Fallback to legacy methods for backward compatibility
      this.logger.warn(`Attempting to create module '${name}' using legacy methods...`);
   ...
   */
  private createModuleLegacy(name: string, config: ModuleFlag): any {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          throw new Error(`Unknown module: ${name}. Please register a plugin for this module.`);
        }
      }
    }
  }
   ...
   */
  private createCoreModuleLegacy(name: string, config: ModuleFlag): any {
    switch (name) {
      case 'auth':
   ...
   */
  private createFeatureModuleLegacy(name: string, config: ModuleFlag): any {
    switch (name) {
      case 'notifications':
   ...
   */
  private createIntegrationModuleLegacy(name: string, config: ModuleFlag): any {
    switch (name) {
      case 'tRPC':
  ...
  public getModuleStatus(): any {
    ...
    return {
      ...
      ocpCompliance: {
        ...
        extensionExample: 'To add a new module, create a class implementing IModulePlugin and register it',
      },
    };
  }
  ...
  public getPluginInfo(): any {
    return {
      ...
      ocpDemonstration: {
        ...
        description: 'This module system demonstrates OCP by allowing extension without modification',
        examples: [
        ...
        architecture: {
          'Plugin Interface': 'IModulePlugin defines the contract for all modules',
          'Plugin Registry': 'ModulePluginRegistryService manages registration and discovery',
          'Module Manager': 'ModuleManagerService delegates creation to registry',
