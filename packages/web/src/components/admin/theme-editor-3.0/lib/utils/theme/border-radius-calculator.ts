...
import { BorderRadiusController, ThemeBorders } from '../../../core/types/theme.types';
...
export function computeBorderValues(borders: Partial<ThemeBorders>): ThemeBorders {
  ...
  const checkboxValue = borders.checkboxRadius?.isLinked ? globalValue : (borders.checkboxRadius?.value ?? globalValue);
  ...
  const radiusCheckbox = `${checkboxValue}px`;
  const radiusCheckboxInner = `${calculateInnerRadius(checkboxValue, COMPONENT_PADDING.CHECKBOX_INNER)}px`;
  ...
  return {
    ...
    checkboxRadius: borders.checkboxRadius ?? createBorderRadiusController(checkboxValue, true, COMPONENT_PADDING.CHECKBOX_INNER),
...
export function updateBorderController(
  ...
  controllerName: 'globalRadius' | 'cardsRadius' | 'buttonsRadius' | 'checkboxRadius',
  newValue: number,
  ...
  if (controllerName === 'globalRadius') {
    ...
    if (updatedBorders.checkboxRadius?.isLinked) {
...
export function toggleBorderLink(
  borders: ThemeBorders,
  controllerName: 'cardsRadius' | 'buttonsRadius' | 'checkboxRadius',
  shouldLink: boolean
): ThemeBorders {
  // Asegurar que tenemos controladores válidos
  const safeBorders = computeBorderValues(borders);
