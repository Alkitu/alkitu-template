...
const replaceAllColorsInSVG = (svgContent: string, newColor: string): string => {
  ...
  const colorNames = [
    ...
    'olive', 'teal', 'silver', 'maroon', 'aqua', 'fuchsia'
  ];
  
  // Atributos que pueden contener colores
  const colorAttributes = ['fill', 'stroke', 'stop-color', 'flood-color', 'color'];
