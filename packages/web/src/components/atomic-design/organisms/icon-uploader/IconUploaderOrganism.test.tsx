...
vi.mock('@/components/atomic-design/atoms/custom-icon', () => ({
...
vi.mock('@/components/atomic-design/atoms/buttons', () => ({
...
vi.mock('@/components/atomic-design/atoms/inputs', () => ({
...
describe('IconUploaderOrganism', () => {
  ...
  const createMockSVGFile = (name = 'test-icon.svg', content = '<svg></svg>') => {
  ...
  describe('Rendering', () => {
    ...
    it('renders default title and description', () => {
      ...
      expect(screen.getByText('Upload Custom Icon')).toBeInTheDocument();
      expect(screen.getByText('Upload an SVG icon to add to your icon library.')).toBeInTheDocument();
      expect(
        screen.getByText('SVG files will be processed and optimized for the theme system.'),
      ).toBeInTheDocument();
    });
  ...
  describe('File Selection', () => {
    it('accepts SVG file selection', async () => {
      ...
      const file = createMockSVGFile('my-icon.svg', '<svg><path d="M0 0"/></svg>');
  ...
  describe('SVG Validation', () => {
    it('shows preview for valid SVG file', async () => {
      ...
      const file = createMockSVGFile('icon.svg', '<svg><circle cx="10" cy="10" r="5"/></svg>');
      const fileInput = screen.getByLabelText('SVG file input');
  ...
  describe('Preview Functionality', () => {
    ...
    it('shows preview with color variants', async () => {
      ...
      await waitFor(() => {
        const icons = screen.getAllByTestId('custom-icon');
        const variants = icons.map((icon) => icon.getAttribute('data-variant')).filter(Boolean);
  ...
  describe('Icon Name Input', () => {
    ...
    it('shows helper text for icon name input', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Only letters, numbers, dashes and underscores allowed'),
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Upload Functionality', () => {
    ...
    it('closes modal after successful upload', async () => {
      ...
      const mockUpload = vi.fn().mockResolvedValue(undefined);
      render(
        <IconUploaderOrganism {...defaultProps} onClose={mockClose} onUpload={mockUpload} />,
      );
  ...
  describe('Accessibility', () => {
    ...
    it('associates helper text with icon name input', async () => {
      ...
      await waitFor(() => {
        const nameInput = screen.getByLabelText('Icon Name');
        expect(nameInput).toHaveAttribute('aria-describedby', 'icon-name-helper');
      });
    });
