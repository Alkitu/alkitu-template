...
import {
  ...
  applyShadowElements,
} from '../../lib/utils/css/css-variables';
...
type ThemeEditorAction =
  ...
  | {
      ...
      payload: {
        mode: ThemeMode;
        colors: import('../types/theme.types').ThemeColors;
      };
    }
...
// Reducer
function themeEditorReducer(
  state: ThemeEditorState,
  action: ThemeEditorAction,
): ThemeEditorState {
  switch (action.type) {
    case 'SET_THEME':
      const newCurrentTheme = computeCurrentTheme(
        action.payload,
        state.themeMode,
      );
      return {
    ...
    case 'SET_THEME_MODE':
      const updatedCurrentTheme = computeCurrentTheme(
        state.baseTheme,
        action.payload,
      );
      return {
    ...
    case 'UNDO':
      ...
      const undoCurrentTheme = computeCurrentTheme(
        undoResult.entry.baseTheme,
        undoResult.entry.themeMode,
      );
      return {
        ...state,
    ...
    case 'REDO':
      ...
      const redoCurrentTheme = computeCurrentTheme(
        redoResult.entry.baseTheme,
        redoResult.entry.themeMode,
      );
      return {
        ...state,
...
export function ThemeEditorProvider({
  ...
  const companyId =
    propCompanyId && propCompanyId !== HARDCODED_ID
      ? propCompanyId
  ...
  const { data: dbThemes, refetch: refetchThemes } =
    trpc.theme.listAllThemes.useQuery(undefined, {
      refetchOnMount: true,
  ...
  useEffect(() => {
    ...
    if (initialTheme) {
      ...
      console.log(
        '🎨 [ThemeContext] Using initialTheme from SSR:',
        initialTheme.name,
      );
    }
    ...
    // Priority 3 (LOWEST): Use hardcoded default as last resort
      ...
      const initialColors =
        state.themeMode === 'dark'
          ? DEFAULT_THEME.darkColors
          : DEFAULT_THEME.lightColors;
    ...
    if (loadedTheme.shadows && Object.keys(loadedTheme.shadows).length > 0) {
      applyShadowElements(
        loadedTheme.shadows as unknown as Record<string, string>,
      );
    }
  ...
  const setEditorSection = (section: EditorSection) =>
    dispatch({ type: 'SET_EDITOR_SECTION', payload: section });
  const setViewport = (viewport: ViewportSize) =>
    dispatch({ type: 'SET_VIEWPORT', payload: viewport });
  const setPreviewSection = (section: PreviewSection) =>
    dispatch({ type: 'SET_PREVIEW_SECTION', payload: section });
  ...
  const markSaved = () =>
    dispatch({ type: 'TOGGLE_UNSAVED_CHANGES', payload: false });
