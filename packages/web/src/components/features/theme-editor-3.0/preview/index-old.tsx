...
export function Preview() {
  ...
  // Simple scroll preservation that only runs when viewport or section changes
  useEffect(() => {
    // Only preserve/restore scroll when switching viewports or sections, not on theme changes
    const currentViewport = state.viewport.current;
    const activeSection = state.preview.activeSection;
    
    if (currentViewport === 'desktop' || currentViewport === 'tv') {
      const currentEl = desktopScrollRefs.current[activeSection];
      if (currentEl) {
        restoreScroll(`desktop-${activeSection}`, currentEl);
      }
    } else if (currentViewport === 'tablet') {
      const currentEl = tabletScrollRefs.current[activeSection];
      if (currentEl) {
        restoreScroll(`tablet-${activeSection}`, currentEl);
      }
    } else if (currentViewport === 'smartphone') {
      const currentEl = smartphoneScrollRefs.current[activeSection];
      if (currentEl) {
        restoreScroll(`smartphone-${activeSection}`, currentEl);
      }
    }
  }, [state.viewport.current, state.preview.activeSection, restoreScroll]);
  ...
  function TVDesktopContent() {
    ...
    // Create stable scroll preservation handlers for desktop/TV using useCallback
    const handleScrollRef = useCallback((section: string) => (el: HTMLDivElement | null) => {
      if (el) {
        // Set new ref directly without interfering with existing scroll position
        desktopScrollRefs.current[section] = el;
      }
    }, []); // Empty dependency array to keep refs stable
  ...
  function TabletContent() {
    ...
    // Create stable scroll preservation handlers for tablet using useCallback
    const handleTabletScrollRef = useCallback((section: string) => (el: HTMLDivElement | null) => {
      if (el) {
        // Set new ref directly without interfering with existing scroll position
  ...
  function SmartphoneContent() {
    ...
    // Create stable scroll preservation handlers for smartphone using useCallback  
    const handleSmartphoneScrollRef = useCallback((section: string) => (el: HTMLDivElement | null) => {
      if (el) {
        // Set new ref directly without interfering with existing scroll position
